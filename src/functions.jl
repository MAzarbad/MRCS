...
function hjbsolver_delta(;
    ...
    MIN =
        bboptimize(H0; SearchRange = SearchRanges, TraceMode = :silent, MaxSteps = MaxSteps)
...
function hjbsolver_delta1(; NRe, β, FR, pR, End, Δx, Re)
    xv = 0:Δx:End
    n = length(xv)
    v = zeros(n + 1)
    vhat = zeros(n)
    ...
    return collect(xv[1:n]), v[1:n] ./ v[n], vhat[1:n] ./ v[n]
end
